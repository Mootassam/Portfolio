...
class ContactRepository {
  static async create(data, options: IRepositoryOptions) {
    ...
    const [record] = await Contact(options.database).create(
      [
        {
          ...data,
  ...
  static async update(
    ...
    let record =
      await MongooseRepository.wrapWithSessionIfExists(
        Contact(options.database).findOne({
          _id: id,
          tenant: currentTenant.id,
        }),
        options,
    ...
    await Contact(options.database).updateOne(
      { _id: id },
      {
        ...data,
        updatedBy:
  ...
  static async destroy(id, options: IRepositoryOptions) {
    ...
    await Contact(options.database).deleteOne(
      { _id: id },
      options,
    );
